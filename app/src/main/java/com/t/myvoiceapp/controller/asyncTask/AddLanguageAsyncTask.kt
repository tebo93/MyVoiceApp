...
class AddLanguageAsyncTask(var c: SeeChooseAddLanguageController) : AsyncTask<String, String, String>() {
    ...
    override fun onPostExecute(result: String?) {
        try {
            ...
            c.resultAddedCorrectly(json.getString("result"))
        } catch (e: Exception) {
            try {
                val json = JSONObject(result)
    ...
    private fun connectionSetProperties(conn: HttpURLConnection, params: Array<out String?>) {
        ...
        conn.setRequestProperty("x_langue", params[3])
        conn.doInput = true
    }
}